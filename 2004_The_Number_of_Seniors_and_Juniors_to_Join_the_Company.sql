...
WITH CTE AS
 ...
            sum(salary) OVER(
                PARTITION BY experience ORDER BY salary) as rnk
    FROM Candidates)
(SELECT 'Senior' as experience, count(employee_id) as accepted_candidates
FROM CTE
WHERE rnk <= 70000 AND experience = 'Senior')
UNION
(SELECT 'Junior' as experience, count(employee_id) as accepted_candidates
FROM CTE
WHERE rnk <= (70000- (SELECT COALESCE(max(rnk),0) FROM CTE WHERE rnk <=70000 AND experience = 'Senior')) 
